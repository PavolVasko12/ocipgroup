<footer id="desktopVersionFooter">
          <div id="footerHolderInner">
            <p><strong>&copy; OCIP GROUP</strong> 2017. All rights reserved.</p>
              <ul id="footerUl">
                   <a class="anchorInTheNavigationMenu" href="index.php" alt="OCIP - Home Page"><li class="footerListClass">Home</li></a>
                   <a href="index.php#holderGetStarted" id="servicesFooterListId" class="anchorInTheNavigationMenu" alt="OCIP - services link"><li class="footerListClass">Services</li></a>
                    <a href="index.php#holderForOurRates" id="priceFooterLink"><li class="footerListClass">Prices</li></a>
                   <a class="anchorInTheNavigationMenu" href="all-galleries.php" alt="OcipGroup - Our Work"><li class="footerListClass">Our Work</li></a>
                   <a hre="#" alt="Blog page Ocip-group"><li class="footerListClass">Blog</li></a>
                   <a href="ocip-contactPage.php" alt="Ocip contact Page"><li class="footerListClass">Contact</li></a>
                   <a href="whereAreWePage.php" alt="Where are we page Ocip-Group"><li class="footerListClass">Where are we?</li></a>
                   <a href="site-map.php" alt="OcipGroup - Sitemap"><li class="footerListClass">Sitemap</li></a>
              </ul>
              <object>
                   <embed id="footerLogo" src="MEDIA/footerOcipLogo.svg">
              </object>
          </div>
      </footer>
    <!------------------------------ FOOTER FROM HERE BELOW 800PX ----------------------->           
       <footer id="smallVersionFooter">
          <div id="footerHolderInner">
              <object>
                    <embed id="footerLogo" src="MEDIA/footerOcipLogo.svg">
              </object>
              <ul id="footerUl">
                   <a class="anchorInTheNavigationMenu" href="index.php" alt="OCIP - Home Page"><li class="footerListClass">Home</li></a>
                   <a href="index.php#holderGetStarted" id="servicesFooterListId" class="anchorInTheNavigationMenu" alt="OCIP - services link"><li class="footerListClass">Services</li></a>
                    <a href="index.php#holderForOurRates" id="priceFooterLink"><li class="footerListClass">Prices</li></a>
                   <a class="anchorInTheNavigationMenu" href="all-galleries.php" alt="OcipGroup - Our Work"><li class="footerListClass">Our Work</li></a>
                   <a hre="#" alt="Blog page Ocip-group"><li class="footerListClass">Blog</li></a>
                   <a href="ocip-contactPage.php" alt="Ocip contact Page"><li class="footerListClass">Contact</li></a>
                   <a href="whereAreWePage.php" alt="Where are we page Ocip-Group"><li class="footerListClass">Where are we?</li></a>
                   <a href="site-map.php" alt="OcipGroup - Sitemap"><li class="footerListClass">Sitemap</li></a>
              </ul>
              <p><strong>&copy; OCIP GROUP</strong> 2017. All rights reserved.</p>
          </div>
      </footer>